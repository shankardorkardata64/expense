<?php
 ...
 {
    $CI=&get_instance();
    return @$CI->db->get_where('users',array('id'=>$id))->result_array()[0];
 }
 ...
 {
    $CI=&get_instance();
    return $CI->db->get_where('setting',array('name'=>$name))->row()->value;
 }
 
 function approve_ex($id)
 {
    $CI=&get_instance();
    $CI->db->where('id',$id);
    $CI->db->update('expences',array('staus'=>1,'updated_at'=>time()));
 }
