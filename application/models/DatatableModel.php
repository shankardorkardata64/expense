...
function getwalletlogjson($table,$searchQuery='',$postData=null,$id='')
{
 $response = array();
   $table='wallet_log';
 ## Read value
 ...
 if($role==4)
{
 ...
 foreach($records as $record ){
   ...
   // if($added_by==$this->session->userdata('id'))
   // {
   //    $added_by='Self';
   // }
   // else
   // {
      $addedrole=$this->db->get_where('users',array('id'=>$added_by))->row()->role;
      
      $added_by=$this->db->get_where('usertype',array('id'=>$addedrole))->row()->name.'('.$this->db->get_where('users',array('id'=>$added_by))->row()->username.')';
   // }

    $data[] = array( 
       "srno"=>1,
       "name"=>$this->db->get_where('users',array('id'=>$record->user_id))->row()->username,
       "before_amount"=>"Rs ".$record->before_amount,
       "action"=>$record->action,
       "amount"=>"Rs ".$record->amount,
       "new_amount"=>"Rs ".$record->new_amount,
       "note"=>$record->note,
       "date"=>$record->date,
       "added_by"=>$added_by,
    ); 
 ...
 return $response; 
   ...
   function getuserjson($table,$searchQuery='',$postData=null,$id='')
   {
    ...
    ## Total number of records without filtering
    $this->db->select('count(*) as allcount');
    $records = $this->db->get_where($table,array('is_deleted'=>0))->result();
    $totalRecords = $records[0]->allcount;
   ...
   $this->db->or_like('empcode',$searchValue);
    ...
    foreach($records as $record ){
       ...
       $data[] = array( 
          ...
          'link2'=>"<a href=".base_url('status-emp').'/'.en($record->id).">$s</a>",
           'link3'=>"<a href=".base_url('delete-emp').'/'.en($record->id).">Delete</a>"
       ); 
    }
   ...
   $this->db->or_like('note',$searchValue);
    ...
    ## Fetch records
    $this->db->select('users.username,expences_type.name as type,expences.amount,expences.wallet_type,expences.user_id,expences.file,expences.id,expences.rejectnote,expences.edate,expences.staus');
    if($searchValue != '') 
    {
      $this->db->like('amount',$searchValue);
      $this->db->or_like('note',$searchValue);
    ...
    foreach($records as $record )
    {
       ...
       if($this->session->userdata('id')==$record->user_id)
         {
...
if($record->file!=NULL)
{
   $file='<a href="'.base_url("uploads/").$record->file.'"  target="_blank">View Attachment</a>';
}

       $data[] = array( 
          "srno"=>1,
          "username"=>$record->username,
          "type"=>$record->type,  "wallet_type"=>$record->wallet_type,
          "amount"=>'Rs '.number($record->amount),
          "edate"=>$record->edate,
          "file"=>$file,
          "status"=>$status,
          'link2'=>$link2,
   ...
   function getexpencesjson1($table,$searchQuery='',$postData=null,$id='')
   {
    $response = array();
    $table='expences';
    ## Read value
